function DAE = RClineDAEAPIv6(uniqIDstr,nsegs,lineR,lineC)  % DAEAPIv6.2
%function DAE = RCline(uniqIDstr,nsegs, lineR, lineC) 
% RC line circuit
%Authors: 
%       - T. Wang, 2013/09/28 (init/limiting related updates)
%       - J. Roychowdhury, 2010/12/17-19, 2011/11
% the circuit: 
...
%2010/12/17-19, 2011/11: Jaijeet Roychowdhury <jr@berkeley.edu>
...
%		add to node i KCL: (ei-eim1)/R + C dei/dt
	...
	bvec = zeros(nsegs,1);
	cvec = zeros(1,nsegs); cvec(1,nsegs) = 1; % output = e(last node).  Note: y = cvec * x, not cvec^T * x % according to DAEAPI v6.x
	g = 1/R;
